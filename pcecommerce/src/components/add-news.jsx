...
const AddNews = () => {
  ...
  const featuredCount = newsList.filter(item => item.displaySection === "featured").length;
  const latestCount = newsList.filter(item => item.displaySection === "latest").length;
  const techCount = newsList.filter(item => item.category?.toLowerCase() === "tech").length;
  const gameCount = newsList.filter(item => item.category?.toLowerCase() === "game").length;
              ...
              <div className="sherah-body">
                <div className="sherah-dsinner">
                  ...
                  <div className="news-summary" style={{ margin: "20px 0" }}>
                    <p>Tổng số bài viết: <strong>{totalCount}</strong></p>
                    <p>Bài viết Featured: <strong>{featuredCount}</strong></p>
                    <p>Bài viết Latest: <strong>{latestCount}</strong></p>
                    <p>Tin công nghệ: <strong>{techCount}</strong></p>
                    <p>Tin game: <strong>{gameCount}</strong></p>
                  </div>
                      ...
                      >
                        ...
                        <tbody className="sherah-table__body">
                          {newsList.map((newsItem) => (
                            <tr key={newsItem._id}>
                              <td className="sherah-table__column-1">{newsItem._id}</td>
                              <td className="sherah-table__column-2">{newsItem.title}</td>
                              <td
                                className="sherah-table__column-9"
                                style={{ display: "table-cell", padding: "12px", minWidth: "100px", borderBottom: "1px solid #eee" }}
                              >
                                <span className={getCategoryBadgeClass(newsItem.category)}>
                                  {newsItem.category}
                                </span>
                              </td>
                              <td className="sherah-table__column-3">
                                <input
                                  ...
                                  onChange={() => handleToggleDisplay(newsItem, "isDisplayed")}
                                />
                              </td>
                              <td className="sherah-table__column-5">
                                <select
                                  value={newsItem.displaySection || ""}
                                  onChange={(e) => handleSectionChange(newsItem, e)}
                                >
                              ...
                              <td className="sherah-table__column-8">
                                <button onClick={() => handleDelete(newsItem._id)} className="sherah-btn">
                                  Xóa
                                </button>
                              </td>
                            </tr>
