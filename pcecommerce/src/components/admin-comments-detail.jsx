...
const AdminCommentDetail = () => {
  ...
  const handleEdit = async () => {
    try {
      const res = await axios.put(`${API_URL}/comment/${id}`, { content: editContent });
      setComment(res.data);
  ...
  const handleNotify = async () => {
    ...
    try {
      await axios.put(`${API_URL}/comment/notify/${id}`, { message: notification });
  ...
  const saveReplyEdit = async (replyId) => {
    try {
      await axios.put(`${API_URL}/comment/reply/${replyId}`, { content: replyEdits[replyId].content });
      fetchCommentDetail();
            ...
            <div className="col-12 sherah-main__column">
              <div className="sherah-card mg-top-30">
                ...
                <header className="sherah-card__header">
                  ...
                  <Link to="/admin/admin-comments" className="sherah-btn sherah-btn--back">
                ...
                <div className="sherah-card__body">
                  <div className="sherah-detail__info">
                    ...
                    />
                    <button onClick={handleEdit} className="sherah-btn mg-top-10">
                      Cập nhật bình luận
                    </button>
                    ...
                    {comment.reports && comment.reports.length > 0 && (
                      <div className="mg-top-10">
                        <h4 className="sherah-heading__sub">Báo cáo từ người dùng:</h4>
                        <ul>
                          {comment.reports.map((report, index) => (
                            <li key={index}>
                              <strong>{report.reporter}</strong>: {report.reason}
                            </li>
                          ))}
                        </ul>
                      </div>
                  ...
                  <div className="sherah-review-section">
                    ...
                    {comment.replies && comment.replies.length > 0 ? (
                      comment.replies.map((reply) => (
                        ...
                        >
                          ...
                          {replyEdits[reply._id] && replyEdits[reply._id].isEditing ? (
                            <>
                              <textarea
                                className="sherah-wc__form-input"
                    ...
                    />
                    <button onClick={handleReply} className="sherah-btn mg-top-5">
                      Gửi Reply
                    </button>
                  </div>
                  <hr className="sherah-divider" />
                ...
                <footer className="sherah-card__footer">
                  <button onClick={handleDelete} className="sherah-btn sherah-btn--delete">
                    Xóa Bình Luận
                  </button>
                </footer>
              </div>
