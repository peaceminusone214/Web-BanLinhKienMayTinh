...
function CompareProducts() {
    ...
    const handleCompare = async () => {
        ...
        try {
            ...
            navigate("/comparison-results", { state: { comparisonResult: response.data } }); // Dùng navigate để chuyển hướng
        } catch (err) {
    ...
    const handleFilterChange = useCallback((filteredData) => {
        ...
        filteredData.forEach((product, index) => {
            console.log(`rpoduct ${index + 1}: ${product.product_name || "(không có tên)"}`);
        });
