...
router.get("/get-products", async (req, res) => {
  try {
    ...
    let filter = { deleted: { $ne: true } }; // Thêm điều kiện lọc sản phẩm chưa bị xoá
...
// Route lấy danh sách sản phẩm đã bị xoá (soft deleted)
router.get("/get-deleted-products", async (req, res) => {
  try {
    const { category_id } = req.query;
    let filter = { deleted: true }; // Lọc những sản phẩm đã bị xoá
    ...
    if (products.length === 0) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy sản phẩm đã xoá nào" });
    }
    ...
    res
      .status(500)
      .json({ message: "Lỗi máy chủ khi lấy danh sách sản phẩm đã xoá" });
  }
});
...
router.get("/get-product-names", async (req, res) => {
  try {
    const { category_id } = req.query;
    let filter = { deleted: { $ne: true } }; // Thêm điều kiện để chỉ lấy sản phẩm chưa bị xoá
...
router.delete("/delete-products", casbinMiddleware, async (req, res) => {
  ...
  try {
    // Đặt deleted: true cho nhiều sản phẩm hoặc chỉ 1 sản phẩm nếu mảng chỉ chứa 1 phần tử
    ...
    res.status(200).json({
      message: `${updatedProducts.modifiedCount} sản phẩm đã được đánh dấu là đã xóa thành công`,
    });
  } catch (err) {
    console.error("Lỗi:", err);
...
router.put("/restore-products", casbinMiddleware, async (req, res) => {
  ...
  try {
    // Khôi phục nhiều sản phẩm hoặc chỉ 1 sản phẩm nếu mảng chỉ chứa 1 phần tử
    const updatedProducts = await Product.updateMany(
      { _id: { $in: ids }, deleted: true }, // Lọc các sản phẩm đã bị xóa
      { $set: { deleted: false } } // Đặt lại deleted thành false
    );

    // Nếu không có sản phẩm nào được khôi phục
    if (updatedProducts.modifiedCount === 0) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy sản phẩm nào để khôi phục" });
    }

    // Trả về thông báo thành công
    res.status(200).json({
      message: `${updatedProducts.modifiedCount} sản phẩm đã được khôi phục thành công`,
    });
  } catch (err) {
    console.error("Lỗi:", err);
    res.status(500).json({ message: "Lỗi máy chủ khi khôi phục sản phẩm" });
  }
});
...
router.get("/sub-slugs/:categoryId", async (req, res) => {
  ...
  try {
    ...
    if (!category.subCategories || category.subCategories.length === 0) {
      return res
        .status(404)
        .json({ message: "Danh mục không có subCategories" });
    }
...
router.get("/search", async (req, res) => {
  try {
    ...
    const matchedCategories = await Category.find({ name: searchRegex }).select(
      "_id"
    );
