...
router.delete("/delete-orders", casbinMiddleware, async (req, res) => {
  const { ids } = req.body;
...
router.patch("/restore-orders", casbinMiddleware, async (req, res) => {
  const { ids } = req.body;
...
router.put("/update-order-status/:orderId", casbinMiddleware, async (req, res) => {
  try {
